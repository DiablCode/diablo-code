...
c      write(400,*) time_step, CR1(0,0,0),   10000
...
!      call MPI_INST_PROF(urms1(1:NY),urms_tot1)
      call MPI_INST_PROF(vrms(1:NY),vrms_tot)
!      call MPI_INST_PROF(vrms1(1:NY),vrms_tot1)
      call MPI_INST_PROF(wrms(1:NY),wrms_tot)
!      call MPI_INST_PROF(wrms1(1:NY),wrms_tot1)
...
!      end do
     ...
     &                        -(tke_mean(j)-tke_mean(j-1))/DYF(J-1))
      ...
      do n=1,N_TH
        do j=2,NY
          tke_5(j,n)= RI_TAU(n)*(thv(j,n)*sin(ANG_BETA)-
     &                 thu(j,n)*cos(ANG_BETA))/10.0d0
        end do
      end do 
      bu_pro(:)=0.0d0
      do j=2,NY
        do n=1,N_TH
          bu_pro(j)=bu_pro(j)+tke_5(j,n)/10.0d0
        enddo
      enddo
      bu_pro(1)=bu_pro(2)
...
!      
      ...
      INTEGER i,j,k
      CHARACTER*38 filn_th_xy
      CHARACTER*38 filn_th_yz 
      CHARACTER*38 filn_th_xz_1,filn_th_xz_2,
     &             filn_th_xz_3
      LOGICAL      SAVE_XZ
...
!     &        //CHAR(MOD(j,10)+48) //
!     &        '.pln'
!       filn_th_xy = 'plane_data/data_xy_n'
!     &         //CHAR(MOD(I,100)/10+48)
!     &        //CHAR(MOD(I,10)+48) // '_'
!     &        //CHAR(MOD(j,10000000)/1000000+48)
!     &        //CHAR(MOD(j,1000000)/100000+48)
...
!     &        '.pln'
       ...
       do i=1,NX
         CALL MPI_INST_PROF(dble(U3(i,0,1:NY)),U3_t(1:NY*NPROCS,i))
         CALL MPI_INST_PROF(dble(U1(i,0,1:NY)),U1_t(1:NY*NPROCS,i))
         CALL MPI_INST_PROF(dble(U2(i,0,1:NY)),U2_t(1:NY*NPROCS,i))
         CALL MPI_INST_PROF(dble(TH(i,0,1:NY,1)),TH_t(1:NY*NPROCS,i))
         CALL MPI_INST_PROF(dble(TH(i,0,1:NY,2)),S_t(1:NY*NPROCS,i))
       enddo
       
       do k=1,NZ
         CALL MPI_INST_PROF(dble(U1(0,k,1:NY)),U1_ty(1:NY*NPROCS,k))
         CALL MPI_INST_PROF(dble(U2(0,k,1:NY)),U2_ty(1:NY*NPROCS,k))
         CALL MPI_INST_PROF(dble(U3(0,k,1:NY)),U3_ty(1:NY*NPROCS,k))
         CALL MPI_INST_PROF(dble(TH(0,k,1:NY,1)),TH_ty(1:NY*NPROCS,k))
         CALL MPI_INST_PROF(dble(TH(0,k,1:NY,2)),S_ty(1:NY*NPROCS,k))
       enddo
       do i=1,NX
         do k=1,NZ
          U2m(k,i)=dble(U2(i,k,30))
         enddo
       enddo
!       CALL MPI_INST_PRO(TH(1:NX,0,1:NY,1),TH_t(1:NY*NPROCS,1:NX))
!       CALL MPI_INST_PRO(dble(U2(0:NXM,0,1:NY)),U2_t)
!      CALL MPI_INST_PRO(dble(U3(0:NXM,0,1:NY)),U3_t(1:NY*NPROCS,0:NXM))
!       CALL MPI_INST_PRO(dble((0:NXM,0,1:NY)),TH)
!      open(543,file=filn_th_xy,status='unknown',form='unformatted')
!      open(544,file=filn_th_yz,status='unknown',form='unformatted')

       IF (RANK == 0) THEN 
!       CALL plane_parav_vel_wr_xz(j)
!       CALL plane_parav_vel_wr_yx(j)
!       IF(SAVE_XZ) THEN
       CALL plane_parav_vel_wr_yz(j)
!        SAVE_XZ=.false.
!       ENDIF
!           write(543)TIME
!           write(543)((TH(i,0,j,1),i=0,NXM),j=1,NY),
!           write(543)((U1_t(i,j),i=0,NXM),j=1,NY*NPROCS)
!           write(543)((U2_t(i,j),i=0,NXM),j=1,NY*NPROCS)
!           write(543)((U3_t(j,i),j=1,NY*NPROCS),i=0,NXM)
!     &     ((TH(i,NZM,j,1),i=0,NXM),j=1,NY)
!           write(543)((U1(i,0,j),i=0,NXM),j=1,NY), 
!!     &      ((U2(i,0,j),i=0,NXM),j=1,NY),
!     &      ((U3(i,0,j),i=0,NXM),j=1,NY), 
!     &      ((TH(i,0,j,1),i=0,NXM),/j=1,NY),
!     &      ((TH(i,0,j,2),i=0,NXM),j=1,NY)
           do j=1,NY*NPROCS 
           write(657,409) GYF_TOT(j),U1_t(20,j),U2_t(20,j),U3_t(20,j)
           write(6,*) GYF_TOT(j)
           enddo
           close(657)
!           do j=1,NY*NPROCS
!            write(656,409) real(LX/dble(NX-1)*dble(i-1)), U3_t(i,2),    
!     &       U3_t(i,20),U3_t(i,200)
!             write(656,*) GYF_TOT(j),U1_ty(j,20)
!           enddo
!           close(656)
!           close(543)
       ENDIF
...
!          IF (RANK == 0) THEN
!           DO I=0,NXM
!            WRITE(655,*) GX(I), TH(I,0,1,1), TH(I,0,1,2)
!           ENDDO
!          close(655)
!          ENDIF
...
!           write(544)((TH(0,k,j,1),k=0,NZM),j=1,NY),
!     &     ((TH(NXM,k,j,1),k=0,NZM),j=1,NY)
!          write(544)((U1(0,k,j),k=0,NZM),j=1,NY),
!    &     ((U2(0,k,j),k=0,NZM),j=1,NY),
!    &     ((U3(0,k,j),k=0,NZM),j=1,NY),       
!    &     ((TH(0,k,j,1),k=0,NZM),j=1,NY),
!    &     ((TH(0,k,j,2),k=0,NZM),j=1,NY)
...
!        j = 2 
!        open(545,file=filn_th_xz_1,status='unknown',form='unformatted')

!         write(545)TIME
!         write(545)((U2(i,k,j),i=0,NXM),k=0,NZM)
...
!        j = 37
!        open(546,file=filn_th_xz_2,status='unknown',form='unformatted')
!
!         write(546)TIME
!         write(546)((TH(i,k,j,1),i=0,NXM),k=0,NZM)
...
!        j = 70
!        open(547,file=filn_th_xz_3,status='unknown',form='unformatted')

!         write(547)TIME
!         write(547)((TH(i,k,j,1),i=0,NXM),k=0,NZM)
...
!      implicit none
       ...
       integer i,j,k,jj
       real*8     zcor(NX)
       real*4     var_1(1:3*NX*(NY*NPROCS-(NPROCS-1)))
       !FILENAMES
       ...
       np1=NX
       np2= NY*NPROCS-(NPROCS-1)
!       write(6,*)'I am here',np1,np2
!       character(len=100) :: InFileName, OutFileName
!       character(len=100) :: dataDIR, outputDIR
!        real*8     zcor(512)
!         real*8             :: var_th(np2,np1), var_sal(np2,np1)
       do i= 1,np1
         zcor(i)=LX/dble(NX-1)*dble(i-1)
       enddo


        fil_tim   = 'plane_data/data_mm_xy'
!     &         //CHAR(MOD(I,100)/10+48)
!     &        //CHAR(MOD(I,10)+48) // '_'
...
!      write(OutFileName,'(a,a,i5.5,a4)') trim(outputDIR),trim(basename)
!    & //"_n",kk,".vtk"

!       write(6,*) 'Writing to ', OutFileName   
       var_1(:) = 0.0d0 ;
...
!       real*4     var_1(1:3*NX*(NY*NPROCS-(NPROCS-1)))
       !FILENAMES
       character(len=25) :: ss,basename
       real*4     var_2(1:3*NZ*(NY*NPROCS-(NPROCS-1)))
       integer ::  s11,np2,np3
       ...
       fil_time  = 'plane_data/data_th_yz'
     &        //CHAR(MOD(jj,10000000)/1000000+48)
     &        //CHAR(MOD(jj,1000000)/100000+48)
     &        //CHAR(MOD(jj,100000)/10000+48)
     &        //CHAR(MOD(jj,10000)/1000+48)
