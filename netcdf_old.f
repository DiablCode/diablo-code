...
C      retval=nf_def_var(ncid_stats,"U_RMS",NF_DOUBLE,2,dim_1d_t
C     &        ,urms_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval=nf_def_var(ncid_stats,"V_RMS",NF_DOUBLE,2,dim_1d_t
C     &        ,vrms_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval=nf_def_var(ncid_stats,"W_RMS",NF_DOUBLE,2,dim_1d_t
C     &        ,wrms_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
     ...
     &        ,thus_varid)
      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval=nf_def_var(ncid_stats,"ut_prime",NF_DOUBLE,2,dim_1d_t
C     &        ,thvt_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval=nf_def_var(ncid_stats,"us_prime",NF_DOUBLE,2,dim_1d_t
C     &        ,thvs_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
...
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval = nf_put_vara_double(ncid_stats,ubar_varid
C     &             ,nc_start,nc_count,UME_TOT(1:NY_TOT))
C      if (retval .ne. nf_noerr) call handle_err(retval)
C       retval = nf_put_vara_double(ncid_stats,t_varid,
C     &             nc_index, 1, TIME)
      if (retval .ne. nf_noerr) call handle_err(retval)
     ...
     &             ,nc_start,nc_count,WME_TOT(1:NY_TOT))
      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval = nf_put_vara_double(ncid_stats,urms_varid
C     &             ,nc_start,nc_count,vrms_tot(1:NY_TOT))
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval = nf_put_vara_double(ncid_stats,vrms_varid
C     &             ,nc_start,nc_count,vrms_tot(1:NY_TOT))
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval = nf_put_vara_double(ncid_stats,wrms_varid
C     &             ,nc_start,nc_count,wrms_tot(1:NY_TOT))
C      if (retval .ne. nf_noerr) call handle_err(retval)
     ...
     &             ,nc_start,nc_count,thus_t(1:NY_TOT))
      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval = nf_put_vara_double(ncid_stats,thvt_varid
C     &             ,nc_start,nc_count,thvt_t(1:NY_TOT))
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval = nf_put_vara_double(ncid_stats,thvs_varid
C     &             ,nc_start,nc_count,thvs_t(1:NY_TOT))
C      if (retval .ne. nf_noerr) call handle_err(retval)
...
C     associated with the file, and flushes any buffers.
      ...
      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval =nf_inq_varid(ncid_stats,"U_RMS",urms_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval =nf_inq_varid(ncid_stats,"V_RMS",vrms_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval =nf_inq_varid(ncid_stats,"W_RMS",wrms_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
      ...
      if (retval .ne. nf_noerr) call handle_err(retval)
      retval =nf_inq_varid(ncid_stats,"wt_prime",thut_varid)
      if (retval .ne. nf_noerr) call handle_err(retval)
      retval =nf_inq_varid(ncid_stats,"ws_prime",thus_varid)
      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval =nf_inq_varid(ncid_stats,"ut_prime",thvt_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
C      retval =nf_inq_varid(ncid_stats,"us_prime",thvs_varid)
C      if (retval .ne. nf_noerr) call handle_err(retval)
