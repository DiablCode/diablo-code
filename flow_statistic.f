...
!        modification for 2d field mainak............................
!....................................................................
...
!Now the 2d field...........mainak
        uv2d(i,j)=uv2d(i,j)+(U1(i,k,j)-dble(CR1(0,0,j)))
     +    *(0.5*(U2(i,k,j)+U2(i,k,j+1))
     &    -0.5*dble(CR2(0,0,j)+CR2(0,0,j+1)))
        wv2d(i,j)=wv2d(i,j)+(U3(i,k,j)-dble(CR3(0,0,j)))
     +    *(0.5*(U2(i,k,j)+U2(i,k,j+1))
     &      -0.5*dble(CR2(0,0,j)+CR2(0,0,j+1)))
        uw2d(i,j)=uw2d(i,j)+(U1(i,k,j)-dble(CR1(0,0,j)))
     +    *(U3(i,k,j)-dble(CR3(0,0,j)))
        pv2d(j,i)=pv2d(i,j)+(P(i,k,j)-dble(CFTH(0,0,j,1)))
     +    *(0.5*(U2(i,k,j)+U2(i,k,j+1))
     &    -0.5*dble(CR2(0,0,j)+CR2(0,0,j+1)))
      

      end do
!Modicifaction by mainak for 2d turbulentproduction
        uv2d(j,i)=uv2d(j,i)/float(NZ)
        wv2d(j,i)=uv2d(j,i)/float(NZ)
        uw2d(j,i)=uv2d(j,i)/float(NZ)
        pv2d(j,i)=uv2d(j,i)/float(NZ)
!     end do
...
!    initializing 2d thu thv field for buoyancy production calculation......modification by mainak
      thu2d(:,:,:)=0.0d0
      thv2d(:,:,:)=0.0d0
...
! This subroutine applies a sponge relaxation (Rayleigh dmping) towards a
...
!modification by mainak for 2d tke field..............................
      tke2d(:,:)=0.0d0
      do j=1,NY+1
       do i=1,NX
         tke2d(i,j) = 0.5*((DYF(J-1)*urms2d(i,j)**2.d0
...
!        tke_3(j)=-uv(j)*dudy(j)-wv(j)*dwdy(j)
     ...
     &           -wv(j)*(dble(CR3(0,0,j))-dble(CR3(0,0,j-1)))/DY(j)
        ...
        tke_3(NY+1)=tke_3(NY)
        tke_3(1)=tke_3(2)
!Calculating the 2d production
       do i=1,NX
        do j=1,NY
          tke_32d(i,j)=-uv2d(i,j)*(dble(CR1(0,0,j))-dble(CR1(0,0,j-1)))
     &    /DY(j) -wv2d(i,j)*(dble(CR3(0,0,j))-dble(CR3(0,0,j-1)))/DY(j)
        enddo
       enddo
...
!modification by mainak for calculating buoyancy production 2d......
        do i=1,NX
         do j=1,NY
...
!for calculating 2d dissipation term.............mainak............
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       if(rank.eq.0)then
       write(6,*) 'putting 3d velocity field'
       endif
       do k=1,NZ
         do i=1,NX
           CALL MPI_INST_PROF(U1(i,k,1:NY),U1_t(1:NY_TOT,i))
!           CALL MPI_INST_PROF(U2(i,k,1:NY),U2_3(1:NY_TOT,k,i))
!           CALL MPI_INST_PROF(U3(i,k,1:NY),U3_3(1:NY_TOT,k,i))
            U1_3(:,k,i)=U1_t(:,i)
          enddo
!          do j=1,NY_TOT
!            do i=1,NX
...
!          enddo
        enddo
!        open(444,file='temp.dat',status='old',form='unformatted')
         if(rank .eq.0) then
         write(6,*) 'checking dim', NY_TOT, NZ
         do j=1,NY_TOT
            do k=1,NZ
!              do i=1,NX
           !    open(444,file='temp.dat',status='old',form='unformatted')
               write(344,*) U1_3(j,k,10)
!              close(444)
              enddo
!            enddo
         enddo
       ...
       if (rank .eq. 0) then
       write(6,*) 'velocity field is done'
       endif
       do i=1,NX
...
!............................................................
!........accumulating 2d tke variables for side wall (xz plane)..mainak...........
         ...
         CALL MPI_INST_PROF(tke_32d(i,1:NY),prod2d(1:NY_TOT,i))
       enddo
...
!       UME_3(:,:,:)=0.0d0     
       do i=1,NX
          do k=1,NZ
          CALL MPI_INST_PROF(U1(i,k,1:NY),UME_3(1:NY_TOT,i,k))
          enddo
       enddo
!       if (rank.eq.0) then      
!       write(6,*) 'dissipation is', disst(400,5),ep2d(20,5)
!         write(6,*) 'ume 3d is ', UME_3(NY_TOT-12,50,50)
!       endif
...
!........Here I am calling the vtk subroutine for tke
!        CALL plane_parav_tke_xz(TIME_STEP)
!.......Here I am calling the vtk subroutine for 3d vel field
!        CALL plane_parav_tot(TIME_STEP)
        write(6,*) 'total field inspection'
...
!HEADER: note termination with char(10)
        ...
        write(13) "SCALARS production float 1"//char(10)
        write(13) "LOOKUP_TABLE default"//char(10)
        write(13) real(prod2d(1:np2,1:np1))

        close(13)
        return
        end     
         subroutine plane_parav_tot(jj)
...
!HEADER: note termination with char(10)
         ...
         write(ss,fmt='(A10,3I5)') "DIMENSIONS",np2,np3,np1
         write(13) ss//char(10)
         ...
         write(13) "LOOKUP_TABLE default"//char(10)
         write(13) real(U1_3(1:np2,1:np3,1:np1))
         write(13) "SCALARS U2 float 1"//char(10)
         write(13) "LOOKUP_TABLE default"//char(10)
         write(13) real(U2_3(1:np2,1:np3,1:np1))
         write(13) "SCALARS U1 float 1"//char(10)
         write(13) "LOOKUP_TABLE default"//char(10)
          write(13) real(U3_3(1:np2,1:np3,1:np1))
         close(13)
         return
      end
